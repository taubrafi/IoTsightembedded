...
void udelay(int t)
{
	...
	for(i=0; i<t; i++)
	{
		SysCtlDelay(SysCtlClockGet() / 30000000);
	}
}
...
void DS2401_Funcs_set(int data)
{
	if(data)
	{
		GPIOPinTypeGPIOInput(ID_PORT_BASE, IDPIN);
		GPIOPinWrite(ID_PORT_BASE, IDPIN, IDPIN);
	}
	else
	{
		GPIOPinTypeGPIOOutput(ID_PORT_BASE, IDPIN);
		GPIOPinWrite(ID_PORT_BASE, IDPIN, 0);
	}
}
...
{
	GPIOPinTypeGPIOInput(ID_PORT_BASE, IDPIN);
	return	GPIOPinRead(ID_PORT_BASE, IDPIN) / IDPIN;
}
